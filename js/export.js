...
function onJsonRangeChange() {
  ...
  if (val === 'pick') {
    ...
    const keys = Object.keys(weeks).filter(k => monthHasData(weeks[k])).sort().reverse();
...
function exportData() {
  ...
  Object.keys(weeks).forEach(k => {
    ...
    const w = weeks[k];
    if (monthHasData(w)) toExport[k] = w;
  });
  ...
  try {
    ...
    if (mode === 'merge') {
      Object.keys(newData).forEach(k => {
        const dest = getOrCreate(k);
        ['doing', 'planned', 'blocked', 'done', 'cancelled'].forEach(col => {
          mergeTasks(dest[col], newData[k][col]);
        });
      });
    } else {
    ...
    init();
    showToast(mode === 'merge' ? 'Data merged successfully.' : 'Data imported successfully.', 'success');
  } catch (e) {
...
function generatePdf() {
  ...
  if (range === 'current') keys = [currentKey];
    ...
    keys = Object.keys(weeks).filter(k => monthHasData(weeks[k])).sort();
  } else {
...
function buildPdfHtml(keys,theme){
  ...
  const weekSections=keys.map(k=>{
    ...
    // Do not use page-break-inside:avoid on the whole month — a busy month is taller
    // than one A4 page, and browsers then shove the entire block onto page 2,
    // leaving page 1 with only the report header (looks blank).
    return`<div style="margin-bottom:28px">
      <div style="font-size:22px;font-weight:600;color:${TEXT};margin-bottom:10px;letter-spacing:-.02em;page-break-after:avoid">${getMonthLabelFromKey(k)}</div>
      ${monthChart}
      ${rows}
    </div>`;
  }).join('');
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @media print{
      ...
      .pdf-card{page-break-inside:avoid;break-inside:avoid}
    }
  </style>
  </head><body>
...
function buildAccomplishmentsHtml(keys, theme) {
  ...
  const taskRows = allDone.length === 0
    ...
    : allDone.map(({ it, monthLabel }, idx) => {
        ...
        const days = daysBetween(it.startedDate || it.createdDate, it.completedDate);
        const daysStr = days !== null ? `${days} day${days === 1 ? '' : 's'}` : null;

        const meta = [
          it.completedDate ? `<span>Completed: <strong>${it.completedDate}</strong></span>` : '',
          it.startedDate   ? `<span>Started: <strong>${it.startedDate}</strong></span>` : '',
          it.createdDate   ? `<span>Added: <strong>${it.createdDate}</strong></span>` : '',
