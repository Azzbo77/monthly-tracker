function renderItem(col, it, i) {
  ...
  const hasProg = it.progress !== null && it.progress !== undefined;
  const progColor = it.progress === 100 ? 'var(--green)' : it.progress >= 50 ? 'var(--blue)' : 'var(--amber)';
  const progHtml = `<div class="prog-wrap" id="prog-wrap-${k}">
    <div class="prog-bar-bg"><div class="prog-bar-fill" id="prog-fill-${k}" style="width:${hasProg ? it.progress : 0}%;background:${progColor};opacity:${hasProg ? '1' : '.25'}"></div></div>
    <input class="prog-card-input" id="prog-card-${k}" type="text" inputmode="numeric" maxlength="3"
      value="${hasProg ? it.progress : ''}" placeholder="0"
      title="Progress 0–100. Setting 100% marks the task complete."
      onclick="event.stopPropagation()"
      oninput="setProgress('${col}',${i},this.value,false)"
      onblur="setProgress('${col}',${i},this.value,false)"
      onkeydown="if(event.key==='Enter'){event.preventDefault();event.stopPropagation();setProgress('${col}',${i},this.value,true);}" /><span class="prog-unit">%</span>
  </div>`;
      ...
      <div class="note-ft">
        <div class="prog-input-wrap">
          ...
          <input class="prog-input" id="prog-${k}" type="text" inputmode="numeric" placeholder="0&#x2013;100"
            ...
            oninput="setProgress('${col}',${i},this.value,false)"
            onblur="setProgress('${col}',${i},this.value,false)"
            onkeydown="if(event.key==='Enter'){event.preventDefault();setProgress('${col}',${i},this.value,true);}" />
          %
        </div>
  ...
  const createdLabel = `<div class="item-dates">
    <span class="item-created">Added ${it.createdDate || '—'}</span>
    <label class="item-started" title="When work actually started — used for time taken on achievements">Started
      <input type="date" class="started-date-in" value="${esc(displayDateToISO(it.startedDate || ''))}"
        onclick="event.stopPropagation()"
        onchange="setStartedDate('${col}',${i},this.value)">
    </label>
  </div>`;
...
function renderResolved(sec, it, i) {
  ...
  const completedDateHtml = !isCan ? `<label class="tag t-date resolved-date" title="Change the date this was actually finished">Completed: <input type="date" class="resolved-date-in" value="${esc(displayDateToISO(it.completedDate || ''))}" onchange="setResolvedDate('done',${i},this.value)"></label>` : '';
  const cancelledDateHtml = isCan ? `<label class="tag t-date resolved-date" title="Change the cancellation date">Cancelled: <input type="date" class="resolved-date-in" value="${esc(displayDateToISO(it.cancelledDate || ''))}" onchange="setResolvedDate('cancelled',${i},this.value)"></label>` : '';
  ...
  return `<div class="item ${isCan?'i-ca':'i-dn'}" data-col="${sec}" data-index="${i}">
    <div class="item-top">
      ${priorityDot}
